...
void author(ts_param *param)
{
	...
	fprintf(g_ts_ts,"==================author info=========================\n");
	//fprintf(g_ts_ts,"网       名:    xiaoC/cabbage/Alexander/Caesar\n");
	fprintf(g_ts_ts,"email:    caiyangang@bupt.edu.cn\n");
	fprintf(g_ts_ts,"blog:    http://blog.csdn.net/cabbage2008\n");
	fprintf(g_ts_ts, "======================================================\n");
}
Anlysis::Anlysis()
{
...
bool Anlysis::anlysisTsHeader()
{
	...
	if(m_param->b_ts)//输出包头信息
	{
		...
		fprintf(g_ts_ts,"Synchronous Tyte:0x%X\n",m_tsHeader.sync_byte);
		fprintf(g_ts_ts,"Error Bit  :%X\n",m_tsHeader.transport_error_indicator);
		fprintf(g_ts_ts,"Starting Bit  :%X\n",m_tsHeader.payload_unit_start_indicator);
		fprintf(g_ts_ts,"Priority  :%X\n",m_tsHeader.transport_priority);
		fprintf(g_ts_ts,"PID     :0x%X ",m_tsHeader.PID);
		switch(m_tsHeader.PID)
		{
		case 0x0000:fprintf(g_ts_ts,"Program Related Table\n");break;
		case 0x0001:fprintf(g_ts_ts,"Conditional Access Table\n");break;
		case 0x0002:fprintf(g_ts_ts,"Transport Flow Description Table\n");break;
		case 0x0003:fprintf(g_ts_ts,"IPMP Control Information Table\n");break;
		case 0x1FFF:fprintf(g_ts_ts,"Empty Package\n",m_tsHeader.PID);break;
		default:
			{
				if(m_tsHeader.PID >= 0x0004 && m_tsHeader.PID <= 0x000F)
				{
					fprintf(g_ts_ts,"Reserved\n");
				}
				else
					fprintf(g_ts_ts,"\n");
			}
		}
		fprintf(g_ts_ts,"Scrambling Method:0x%X ",m_tsHeader.transport_scrambling_control);
		switch(m_tsHeader.transport_scrambling_control)
		{
			case 0x00:fprintf(g_ts_ts,"No Scrambling\n");break;
			case 0x01:fprintf(g_ts_ts,"User Defined\n");break;
			case 0x02:fprintf(g_ts_ts,"User Defined\n");break;
			case 0x03:fprintf(g_ts_ts,"User Defined\n");break;
			default:fprintf(g_ts_ts,"Error in Scrambling Method\n");g_errors++;
		}
		fprintf(g_ts_ts,"Adaptive Word:0x%X ",m_tsHeader.adaptation_field_control);
		switch(m_tsHeader.adaptation_field_control)
		{
		case 0x00:fprintf(g_ts_ts,"For future use, retained by ISO/IEC\n");break;
		case 0x01:fprintf(g_ts_ts,"No adaptation_field, only payload\n");break;
		case 0x02:fprintf(g_ts_ts,"Only Adaptation_field, no payload\n");break;
		case 0x03:fprintf(g_ts_ts,"Adaptation_field followed by payload\n");break;
		default:fprintf(g_ts_ts,"Adaptive Word Error\n");g_errors++;
		}
		fprintf(g_ts_ts,"serial number  :0x%X \n",m_tsHeader.continuity_counter);
...
void Anlysis::anlysisSDT()
{
	...
	*/
	...
	if(m_param->b_ts)
	{
		...
		if(crc!=m_sdt.CRC_32)
		{
			fprintf(g_ts_ts,"error:CRC Verification error:0x%X  0x%X \n",m_sdt.CRC_32,crc);g_errors++;
		}
	}
}
...
void Anlysis::adaptation_field()
{
	...
	if(m_param->b_ts)
	{
		fprintf(g_ts_ts,"Adaptive field parsing\n");
		...
		if(m_adt.adaptation_field_length)
		{
			...
			if(m_adt.PCR_flag)
			{
				...
				{
					__int64 time = m_adt.program_clock_reference_base/90;//m_adt.program_clock_reference_base*300*1000/27000000; //millisecond
					fprintf(g_ts_ts,"%I64d:",time/3600000);time = time%3600000;//honr
					fprintf(g_ts_ts,"%I64d:",time/60000);time = time%60000;//minute
					fprintf(g_ts_ts,"%I64d.",time/1000);time = time%1000;//second
...
void Anlysis::anlysisPES()
{
	...
	if(m_param->b_ts)
	{
		fprintf(g_ts_ts,"PES header analysis\n");
...
void Anlysis::anlysis()
{
	...
	while(1)
	{
		...
		if(m_tsHeader.adaptation_field_control == 2 || m_tsHeader.adaptation_field_control == 3)//Adaptive field parsing
		{
			adaptation_field();
		}

		if(m_tsHeader.adaptation_field_control == 1 || m_tsHeader.adaptation_field_control == 3)//basic dataset
		{
			if(m_tsHeader.PID == m_PATPID) //PAT analysis
			{
				anlysisPAT();
			}
			else if(m_tsHeader.PID == m_PMTPID)//PMT analysis
			{
				anlysisPMT();
			}
			else if(m_tsHeader.PID == m_VideoPID)//video analysis
			{
			...
			else if(m_tsHeader.PID == m_AudioPID)//audio analysis
			{
				unsigned int packet_start_code_prefix = (m_ts.data[m_ts.count]<<16) + (m_ts.data[m_ts.count+1]<<8) + m_ts.data[m_ts.count+2];
				if(packet_start_code_prefix == 0x000001)
					anlysisPES();
			...
			else
			{
				...
				if(m_param->b_ts)
				{
					fprintf(g_ts_ts,"At present, only PAT, PMT, audio and video can be parsed, and other PID cannot be parsed at the moment\n");
				}
			}
		}
		m_tsConter++;
	...
	{
		fprintf(g_ts_ts,"\n\n==================TS Summary Info =========================\n");
		fprintf(g_ts_ts,"TS num:    %d\n",m_tsConter);
		fprintf(g_ts_ts,"TS error num:    %d\n",g_errors);
	}
}